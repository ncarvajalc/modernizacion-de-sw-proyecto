...
WHERE c.nombreoficina = o.nombreeps AND
      ...
      (SELECT TRUNC(TO_NUMBER(SYSDATE - c.fechadenacimiento) / 365.25) FROM DUAL) = 22 AND
...
--Join de todas las condiciones menos la comorbilidad (deberia mandar a juan y rafael)
SELECT distinct c.*
FROM ciudadano c LEFT JOIN pertenecea pe ON
     (c.tipodeidentificacion = pe.tipoidciudadano AND
     c.identificacionciudadano = pe.idciudadano),
...
--Join de todas las condiciones menos la comorbilidad y el genero (debería mandar a juan, rafael y sol)
SELECT distinct c.*
FROM ciudadano c LEFT JOIN pertenecea pe ON
     (c.tipodeidentificacion = pe.tipoidciudadano AND
     c.identificacionciudadano = pe.idciudadano),
